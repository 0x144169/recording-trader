...
- 🐾 **虚拟助手交互重做**：双击恢复随机说话，支持右键菜单，菜单新增显示应用与退出应用。
- 🧭 **设置更清晰**：助手设置标题简化，启用/隐藏合并为「显示助手」单开关，状态表达更直观。
- 🖼️ **头像系统升级**：支持图片皮肤与用户上传头像（可删除），并新增内置「小兔酱」「特朗普」图片皮肤。
- 🔊 **首启欢迎可语音**：新用户首次启动可自动语音欢迎一次，后续遵循语音开关。
- 💾 **迁移保留自定义头像**：导出数据（迁移用）与恢复流程可保留自定义头像与助手配置。
