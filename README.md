...
- 🤖 **AI 分析更稳**：云端 AI 与本地模型链路进一步打磨，单笔 AI 分析在图片模型不可用时会自动降级为纯文字分析。
- 📚 **知识库更清爽**：同一个订单的 AI 分析默认只保留一条，重复保存会覆盖更新，不再堆出多条重复记录。
- 📝 **笔记图片更安全**：知识库手写笔记与富文本中的图片现在会被正确识别为已引用，不会再被“清理孤立图片”误删。
- ⌨️ **交互回归修复**：快捷键串台、小助手遮挡底层交互、输入时误触发其他功能等问题得到系统性修复。
- 🧪 **发布质量加强**：补充更多单元测试与 UI smoke test，并完成 Windows 打包验证。
