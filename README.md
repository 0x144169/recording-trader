...
- 📤 **新增分享下载入口**：更新日志页新增「分享下载链接」按钮，一键复制可转发文案（含版本号 + 下载地址）。
- 💾 **排序/视图偏好持久化**：日报排序、订单总览排序、总览视图（小列表/大卡片）均可跨重启保留。
- 🔁 **老用户平滑兼容**：对旧版 `localStorage` 视图偏好保留回退读取，升级后无需重复设置。
- 🧪 **单测覆盖增强**：新增偏好解析与回退规则测试并接入全量 `npm test`。
- 🗺️ **盘前计划与单前检查持续可用**：保留上一版本多计划盘前工作流与强拦截可编辑录入体验。
